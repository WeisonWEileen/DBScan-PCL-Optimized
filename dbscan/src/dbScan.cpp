...
dbscan::dbscan() : octreeGenIn(new htr::OctreeGenerator()), octreeGen(new htr::OctreeGenerator()) {}
...
void dbscan::calculateCentroid(std::vector<pcl::mod_pointXYZ> &group) {
  ...
  centroid.x /= static_cast<float>(group.size());
  centroid.y /= static_cast<float>(group.size());
  centroid.z /= static_cast<float>(group.size());
}
...
void dbscan::DBSCAN_Octree_merge(htr::OctreeGenerator *octreeGen, float eps, int minPts) {
  ...
  size_t noKeys = clustersAux.size();
...
void dbscan::DBSCAN_Octree(htr::OctreeGenerator *octreeGen, float eps, int minPts) {
  ...
  size_t noKeys = octreeGen->getCloud()->points.size();
  std::vector<bool> visited(noKeys, false);
