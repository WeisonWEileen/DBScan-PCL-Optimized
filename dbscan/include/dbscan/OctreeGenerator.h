...
/// Solution specific to microsoft.
  ...
  template <typename T>
  void initCloudFromVector(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &input_cloud);

  void initOctree(const int resolution);

  void extractPointsAtLevel(const unsigned int depth);
...
template <typename T>
void OctreeGenerator::initCloudFromVector(const pcl::PointCloud<pcl::PointXYZRGB>::Ptr &input_cloud) {
